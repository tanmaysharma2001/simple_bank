...
const createTableUsersPromise = new Promise(async (resolve, reject) => {
    pool.query('create table users(id serial unique not null, first_name varchar(50), last_name varchar(50), email varchar(50) unique not null, password varchar(100), token varchar(200), account_number varchar(30) unique not null, PRIMARY KEY (id, email, account_number));', (error: any, results: any) => {
        if (error) {
            throw error;
        }
    });
...
const createTableAccountsPromise = new Promise(async(resolve, reject) => {
    pool.query('create table accounts(account_number varchar(50) primary key, account_type varchar(30) not null, balance numeric(10, 2) not null);', (error: any, results: any) => {
        if (error) {
            throw error;
        }
    });
